...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  const twimlSuccess = `<Response><Say voice="alice">Thank you. Your daily check-in has been confirmed.</Say></Response>`;
  const twimlFail = `<Response><Say voice="alice">We could not validate your response. Emergency contacts will be notified.</Say></Response>`;
  ...
  await logCheckInEvent(user.id, schedule.id, 'check_in_confirmed', 'ok', schedule.escalation_step, 'Elder care voice confirmation from Twilio');
